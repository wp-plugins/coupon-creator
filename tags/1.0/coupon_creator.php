 ...
 * Description: This plugin creates a custom post type for coupons with a shortcode to display in on site and a single view template for printing. It also includes a button to find and insert the shortcode into the editor. 
 * Author: Brian Jessee
 * Version: 1.0	
 * Author URI: http://jesseeproductions.com
 * License: GPL2

    Copyright 2009-2012  Brian Jessee (email : info@jesseeproductions.com)
...
if ( ! defined( 'COUPON_PLUGIN_URL' ) )
	define( 'COUPON_PLUGIN_URL', WP_PLUGIN_URL . '/coupon_creator' );
 ...
 * 
 */
	include_once 'lib/cc_shortcodes.php';
	...
	if (is_admin()) {
		...
		function add_cc_coupon_button($context) {
		  ...
		  $context .= "<a class='thickbox' title='{$title}'
			href='#TB_inline?width=670&inlineId={$container_id}'>
			<img src='{$img}' /></a>";
		...
		?>
				...
				<script>
					function InsertCoupon(){
						...
						window.send_to_editor("[coupon couponid=\"" + coupon_id + "\" coupon_align=\"" + cctor_align + "\" name=\"" + coupon_name + "\"]");
					}
				</script>
				
			<!--Start Thickbox Popup -->
			<div id="coupon_container" style="display:none;">
			  <h2>Select a Coupon from the List and Insert it into the Editor.</h2>

				<?php
					 $querycoupon = new WP_Query( 'post_type=cctor_coupon&posts_per_page=-1' );
					// The Coupon Loop
					if ($querycoupon->have_posts()) {
				?>
				<div style="padding:15px;">
					<!--Create a Select Box with Coupon Titles -->
					<select name="coupon_select_box" id="coupon_select"> 
 ...
 */		
		...
		add_action('admin_enqueue_scripts','load_admin_coupon_script_style');
		   ...
		   function load_admin_coupon_script_style() {
			...
			wp_enqueue_style('colorbox_jp_css', COUPON_PLUGIN_URL . '/colorbox/colorbox.css', false, '1.0', 'all' );
		...
		$coupon_creator_meta_fields = array(  
			array(  
				'label' => 'Discount',
				'desc' => 'Enter the discount ammount - 30%, Buy One Get One Free, etc...',
				'id' => $prefix . 'amount',
				'type'  => 'text'  
			),
			array(
				'label' => 'Discount Backround Color',
			...
			foreach ($coupon_creator_meta_fields as $field) {  
				...
				echo '<tr> 
						...
						switch($field['type']) {  
							...
							case 'cctor_videos':
								echo "<p>
<a href='http://www.youtube.com/embed/X5xh4DFlZIU?hd=1&autohide=1&rel=0&showsearch=0&autoplay=1' class='youtube_colorbox' rel='how_to_videos'>How to create a Coupon</a><br/>
<a href='http://www.youtube.com/embed/X5xh4DFlZIU?hd=1&autohide=1&rel=0&showsearch=0&autoplay=1' class='youtube_colorbox' rel='how_to_videos'>How to create an image coupon</a><br/>
<a href='http://www.youtube.com/embed/lFq2Wd6_8jk?hd=1&autohide=1&rel=0&showsearch=0&autoplay=1' class='youtube_colorbox' rel='how_to_videos'>How to insert and align coupons</a>
<br/>(click to open)</p>";
							break;
						} //end switch  
				echo '</td></tr>';  
		...
		function save_coupon_creator_meta($post_id) {  
			...
			// verify nonce  
			if (!wp_verify_nonce($_POST['coupon_creator_meta_box_nonce'], basename(__FILE__)))  
				return $post_id;  
			// check autosave  
